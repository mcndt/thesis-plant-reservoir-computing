...
class ExperimentDataset:
    ...
    def __init__(self, dataset_df=None, csv_path=None, max_step=1000):
    ...
    def cache_state(self) -> tuple:
        states = self._state
        n_runs = self.n_runs()
        n_steps = self.n_steps() if self._max_step is None else self._max_step
        ...
        for (run_id, state_id, state_type), run_df in runs_df:
            ...
            for i_var, var in enumerate(state_vars):
                try:
                    self._state_nd[i_run, :, i_node, i_var] = run_df.iloc[:n_steps][var]
                    # self._state_nd[i_run, :, i_node, i_var] = run_df.loc[:n_steps, var]
                except Exception as e:
                    print(
                        n_steps,
                        len(run_df.loc[:n_steps, var]),
                    )
                    raise e
    ...
    def n_steps(self) -> int:
        return (
            ...
            else self._inputs.groupby("run_id").size()[0]
        )
