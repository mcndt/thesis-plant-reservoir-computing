...
from src.post_processing.hydroshoot.extract_vertex_properties import (
    get_time_series_for_class,
)
...
def process_experiment(path: str, name: str, logger=None) -> str:
  ...
  """
    ...
    if logger:
        logger.info(
            f"[{name}] Extracted {len(timestamps)} steps, {len(store)} properties, {len(list(store.values())[0])} state size"
        )
    else:
        print(
