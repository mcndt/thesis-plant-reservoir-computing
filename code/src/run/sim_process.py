...
from src.post_processing.hydroshoot.process_experiment import process_experiment
from src.constants import HYDROSHOOT_DATETIME_FORMAT


def run_experiment(
    path, exp_name, interval: Tuple[Arrow, Arrow] = None, meteo_file=None
):
  ...
  To use a different meteo file than the one present in the template path,
  use the meteo_file kwarg.
  """
    success = False
    # configure_logging()
    logger = get_logger()
    ...
    check_path(path)
    tmp_dir = make_tmp_dir(path, meteo_file=meteo_file)
...
def make_tmp_dir(path, meteo_file=None):
    ...
    copytree(
        path,
        tmp_path,
        ignore=ignore_patterns("output", "*.ipynb", ".ipynb_checkpoints"),
    )
    get_logger().info(f"Created temp directory: {tmp_path}")

    if meteo_file is not None:
        copyfile(meteo_file, os.path.join(tmp_path, "meteo.input"))
        get_logger().info(f"Copied custom meteo file: {meteo_file}")
  ...
  new start and end time."""
    ...
    if not os.path.isfile(params_path):
        raise FileNotFoundError(
            f"Simluation directory has no params file: '{params_path}'"
        )
    ...
    get_logger().info(
        f"Wrote simulation interval {start.format(HYDROSHOOT_DATETIME_FORMAT)}-{end.format(HYDROSHOOT_DATETIME_FORMAT)} to params.json"
    )
